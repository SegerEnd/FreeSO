    ...
    public class TTAB : IffChunk
    {
        ...
        public override bool Write(IffFile iff, Stream stream)
        {
            using (var io = IoWriter.FromStream(stream, ByteOrder.LITTLE_ENDIAN))
            {
                ...
                io.WriteUInt16((ushort)((iff.TS1) ? 8 : 10)); //version. we save version 10 which uses the IO proxy
                //...but we can't write out to that yet so write with compression code 0
                if (!iff.TS1) io.WriteByte(0);
                for (int i = 0; i < Interactions.Length; i++)
                {
                    ...
                    if (!iff.TS1) io.WriteUInt32((uint)action.Flags2);
                }
            }
            return true;
        }
